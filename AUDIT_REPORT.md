...
## Update — Dead code removed (§2.8, plus a larger dead subsystem found while verifying it)

All of the following were re-verified as genuinely unreferenced (repo-wide grep, zero hits outside their own definitions) before deletion:

- **`ProjectSelectionViewModel` + `Views/ProjectSelectionDialog.xaml(.cs)`** — never instantiated. Deleted, along with `MainViewModel.SetProjectSelection`/`_projectSelection`/`TryOpenProjectByName` (the only things that referenced it, themselves unreachable as a result).
- **`ChapterConfirmationViewModel`** — `ChapterConfirmationDialog`'s `DataContext` was never set to one; the dialog's real logic lives entirely in its own code-behind. Deleted the class and the dead `ViewModel` property that cast `DataContext` to it.
- **`AiService.SuggestImprovementAsync`/`AiSuggestion`/`ExtractSection`** — a full prompt+API round trip with no caller anywhere. Deleted, along with the `IsStub` field on both `AiFeedback` and the now-deleted `AiSuggestion` (never set `true` anywhere, dead either way).
- **`AppSound.Success`** — defined and synthesized, never played. Removed the enum value and its synth case.
- **The entire "five-step guided tour" subsystem turned out to be larger than originally scoped** — tracing `Services/TutorialService.cs`'s only entry point (`MainViewModel.StartGuidedTour`) found it has zero callers anywhere in the codebase, which cascaded into a bigger, still fully-dead chain: `MainViewModel.StartGuidedTour`/`OnTourComplete` and its `TourStarted`/`TourEnded` events, `Services/ITutorialPresenter.cs` (implemented only by `MainWindow`, only for this dead flow), `MainWindow`'s explicit `ITutorialPresenter` implementation block, and `VoiceCommandRouter`'s `_tourService`/`SetTourService` plus the "next step"/"repeat step"/"end tour" command-interception block it guarded (a stale vocabulary that only ever appears in the legacy `DragonCommands/QuickReference.txt`, not either canonical manual). The live 17-step tutorial (`TutorialViewModel`/`TutorialDialog`) is entirely separate and untouched. Deleted `TutorialService.cs` and `ITutorialPresenter.cs` outright, and removed every dead reference to them.
- **Bonus bug found and fixed during this removal**: the generic `cmd.StartsWith("open ")` catch-all in `VoiceCommandRouter.cs` (leftover from the deleted project-selection flow) sat *before* several specific "open X" phrase checks defined later in the same method — including the "open welcome"/"open welcome dialog" phrases added in the previous PR — silently swallowing them into the (also dead) `TryOpenProjectByName` path before they could ever be reached. Removing the dead catch-all fixes this shadowing for free.
...
## Update — Architecture cleanup (partial — see note on the MainViewModel split)

Two of the Tier 4 items were completed and verified by hand (no .NET SDK available in this sandbox to compile-check, so every call site, XAML binding path, and generated command/property name was traced manually before and after each change):

- **§2.6 Duplicate `SanitizeForSpeech`/`SelectBestSapiVoice` logic** — fixed. Extracted both into a new `Services/SpeechTextUtils.cs` static helper; `AudioFeedbackService` and `SystemAnnouncementService` now both call the shared implementation instead of maintaining byte-for-byte-identical private copies. Removed now-unused `System.Linq`/`System.Text.RegularExpressions` usings from `SystemAnnouncementService.cs` (and `System.Linq` from `AudioFeedbackService.cs`) as a result.
- **§2.3 Inconsistent MVVM idiom** — fixed for the three library ViewModels. `PromptLibraryViewModel`, `ResponseCardViewModel`, and `FeedbackLibraryViewModel` previously used hand-rolled `INotifyPropertyChanged` + `Utils.RelayCommand`; all three now use `CommunityToolkit.Mvvm`'s `ObservableObject`/`[ObservableProperty]`/`[RelayCommand]`, with `[NotifyPropertyChangedFor]`/`[NotifyCanExecuteChangedFor]` replacing the manual `Notify()` + `((RelayCommand)X).RaiseCanExecuteChanged()` calls. `Utils/RelayCommand.cs` had exactly these three consumers (confirmed by repo-wide grep) and is now deleted. `MainViewModel`, `ChapterViewModel`, `WholeBookViewModel`, `TutorialViewModel`, and `WelcomeDialogViewModel` were already toolkit-based, so the codebase is now consistently on one MVVM idiom.

**§2.2 The `MainViewModel` god-object split — deliberately not attempted.** This was the headline item (splitting the 2,588-line `MainViewModel` into panel-scoped ViewModels), and I want to be transparent about why it isn't in this PR. While tracing the actual dependency graph to plan the AI Assistant panel extraction, I found `SelectedChapter`, `IsWholeBookSelected`, `ChatInputText`, `AiFeedbackText`, and the AI commands (`RunAiFeedbackCommand`, `SendChatCommand`, `InsertAtCursorCommand`, etc.) are cross-wired through `CommunityToolkit.Mvvm`'s `[NotifyCanExecuteChangedFor(nameof(...))]` source-generator attributes (e.g. `SelectedChapter` changing must refresh `RunAiFeedbackCommand`'s enabled state) — and that attribute requires both sides to live in the same partial class to resolve at compile time. Splitting the panels apart would require replacing that compiler-verified wiring with hand-written cross-ViewModel notification calls. Getting that wrong doesn't fail loudly as a compile error — it fails silently as a button or voice command that stays enabled/disabled when it shouldn't, which is exactly the kind of bug manual code review and casual manual testing both tend to miss. Attempting this without a compiler available felt like the wrong tradeoff, so it's left as a known, well-scoped open item rather than guessed at. A real attempt should be done with compiler feedback available at each step, ideally incrementally (one property/command boundary at a time, compiling between each) rather than as one large cut.
