...
namespace VoiceBookStudio.Services
{
    ...
    public class ProjectService
    {
        ...
        /// <summary>
        /// Creates the standard subfolder structure inside a project root folder.
        /// Safe to call on open as well as on create — Directory.CreateDirectory
        /// is idempotent. Failures are swallowed silently so a permission error or
        /// read-only drive never crashes the app.
        /// </summary>
        public static void EnsureProjectFolderStructure(string projectRootPath)
        {
            try
            {
                Directory.CreateDirectory(Path.Combine(projectRootPath, "Chapters"));
                Directory.CreateDirectory(Path.Combine(projectRootPath, "ResponseCards"));
                Directory.CreateDirectory(Path.Combine(projectRootPath, "Feedback"));
                Directory.CreateDirectory(Path.Combine(projectRootPath, "Prompts"));
                Directory.CreateDirectory(Path.Combine(projectRootPath, "Exports"));
                Directory.CreateDirectory(Path.Combine(projectRootPath, "Backups"));
            }
            catch { }
        }
